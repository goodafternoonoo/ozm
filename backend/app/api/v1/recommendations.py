...
async def get_simple_recommendations(
    request: SimpleRecommendationRequest, db: AsyncSession = Depends(get_db)
):
    """
    시간대별 간단 추천 요청
    - 입력: SimpleRecommendationRequest
...
async def get_quiz_recommendations(
    ...
    recommendations = await RecommendationService.get_quiz_recommendations(
        db=db, answers=request.answers, session_id=session_id, limit=5
    )
    return RecommendationResponse(
        recommendations=recommendations,
        session_id=session_id,
